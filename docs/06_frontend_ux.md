...
2. **Maestro-detalle con tabla compacta.** Para colecciones homogéneas: tabla escaneable (patrón familiar tipo hoja de cálculo) → clic abre el detalle de UN elemento a la vez. No usar tarjetas grandes repetidas como listado (impiden escanear). Columnas: lo que responde "¿cuál es cuál?" y "¿qué se movió?" (estado, responsable, conteos, última actividad).
   - **Colecciones que crecen (tablas Y tableros) → tope de altura con scroll interno.** No basta con aplicarlo a las tablas: el Kanban se quedó fuera y con 15 tareas medía 1,5 pantallas, con 40 llegaba a 3,6, y la columna más alta estiraba a las vacías (una con UNA tarjeta medía 1.309px). Con `max-height` + scroll por columna el tablero mide lo mismo con 15 tareas que con 100. **Dos matices aprendidos (2026-07-31):** (a) cuando el layout APILA las columnas (móvil), hay que ANULAR el tope — un contenedor con scroll propio dentro de una página que también se desplaza atrapa el gesto, y el motivo del tope (que una columna no estire a la de al lado) solo existe lado a lado; (b) **una instrucción se pone UNA vez, no en cada elemento**: «Arrastra para cambiar estado» se repetía en cada tarjeta y sumaba ~300px de texto idéntico — va al encabezado de la colección, y en el elemento queda solo lo que es suyo.
...
9. **`text-overflow: ellipsis` NO funciona en el contenedor flex: va en el hijo que lleva el texto (2026-08-04).** En un `display:flex`, el `text-overflow` del padre no alcanza al texto de sus hijos, así que el contenido se corta **en seco, sin los «…»** — y lo que se pierde es lo último del renglón, que suele ser justo el dato que distingue (un conteo, una unidad, una extensión). Regla: el recorte se aplica a un `<span>` propio del nombre, con `min-width: 0` (si no, no encoge por debajo de su contenido), `overflow:hidden`, `text-overflow:ellipsis` y `white-space:nowrap`; a los elementos que NO deben recortarse (icono, conteo, badge) se les pone `flex: none`. Caso real: el chip de carpeta de la bitácora mostraba «🗀 Reporte Reciprocidad Cliente E» y se comía el «89», que era lo único que lo identificaba como carpeta.
10. **Dos alcances distintos NO se apilan en la misma pantalla, y menos con nombres parecidos (2026-08-04).** El detalle de una solicitud tenía «Personas relacionadas» (las de ESA solicitud) y, unos centímetros abajo, «Personas registradas» (las de toda la plataforma): nada avisaba del salto de nivel y se leían como lo mismo. Cuando un bloque cambia de alcance —de un registro al catálogo completo— no basta con renombrarlo: conviene sacarlo a **su propia vista**, hermana de la lista (aquí, un tercer segmento del control Gestión/Tablero). Ahí el alcance se entiende porque cambiaste de vista, no porque leíste bien un título, y se llega en un clic en vez de recorrer todo el detalle. Contrapartida a tener presente: al separarlos se pierden los gestos que cruzaban ambos (arrastrar de uno a otro) — hay que confirmar que la alternativa visible existe antes de mover.
11. **Un contenedor con scroll propio NO puede reconstruirse en cada interacción (2026-08-04).** Si un panel tiene `max-height` + `overflow:auto` y el repintado lo recrea (`innerHTML` del módulo entero), **nace con `scrollTop` 0**: el usuario despliega algo que estaba abajo y la vista lo manda al inicio, obligándolo a volver a bajar para ver lo que acaba de abrir. Regla: los gestos que solo cambian el CONTENIDO de ese panel se repintan **parcialmente** —reemplazar el `innerHTML` del panel, no el del módulo, hace que el nodo sobreviva y conserve su posición— y los que además tocan cosas de afuera (una barra de selección, un contador) hacen el render completo pero **guardan y restauran el `scrollTop`** alrededor. Precedentes en `workspace.ts`: `repintarArbol()` para desplegar carpetas, y el `arbolScroll` de `renderWorkspace` para seleccionar/borrar. Medido: con el árbol a 2687px de scroll, el render completo lo devolvía a 0 y el parcial lo deja intacto.
12. **Un techo en px que “equivale a N filas” caduca en silencio: átalo a la misma fuente que la fila (2026-08-05).** `.peopleStrip` tenía `max-height: 96px` documentado como «2 filas de 42px, nunca se corta un chip a la mitad». La tarjeta creció a 46px (manda el botón de 32 + padding + borde, no el avatar de 28) y 96px pasaron a ser **1.8 filas**: desde entonces SIEMPRE se partía una fila, y nadie lo notó porque el comentario seguía prometiendo lo contrario. Regla: cuando dos medidas tienen que cuadrar entre sí, salen de las mismas variables (`--filaAlto`, `--filaGap`, `--filasVisibles`) y la fila se fija con `grid-auto-rows`, de modo que el techo sea exacto aunque cambie el contenido. Elegir el número de filas de forma que el volumen habitual quepa **sin scroll interno** —con volumen mayor el techo acota, en vez de estirar la página—. Ojo al verificar: a media rueda cualquier lista con scroll muestra filas parciales en el borde; lo que se comprueba es que **en reposo** no corte.
