...
- **Registros (conteo de filas)**: tercera métrica además de Archivos/Peso. NO sale del listado S3 (que solo conoce metadatos): se consulta la **tabla de control de ingesta** `stage_staging.ctl_ingestion_unstructured` (columna `record_count` exacta, cubre **CSV y PARQUET**) vía **Athena**, agregando por **área → tabla** y **área → día**. **Precisión (alineada con el reporte oficial de ingestas)**: (a) **deduplica reprocesos** tomando el `record_count` de la ingesta **más reciente por `target_path`** (`row_number() … order by execution_timestamp desc`), en vez de sumar todas las filas — sin esto se inflaba ~68%; (b) usa la **fecha lógica del archivo** (`archivo_anio/mes/dia`), no la de partición/ingesta. Validado: reconcilia **exacto** con el conteo real de la tabla staging (ej. `divisas_ingresos_egresos` 23/06 = 2,486 en ambos). Misma lógica que `docs`/los queries `ingest_summary_*.sql` del equipo de datos. Athena corre **asumiendo el rol del hub** `gestion-proyectos-cost-reader` (el catálogo, los datos y los resultados viven en el hub); agrega miles de part-files diminutos en **segundos** (≈10s para 90 días) — antes se intentó leer footers/parquet directo pero no escalaba. Acotado al **rango elegido** (no aplica con "Todo") y **cacheado** por `bucket#zone#inicio#fin` (si está en caché se sirve de ahí; si no, se calcula async con poll). El nombre de **tabla** se deriva de `target_path` (staging) o `s3_key` (landing): ruta antes de la primera partición `clave=valor` (staging Hive) o filename sin fecha (landing). Filtra a `bucket_name='arc-enterprise-data'` para excluir ingestas de sandbox. **Importante**: en la pestaña Registros, las columnas **Ingestas** y **Peso origen** vienen del log de ingesta (eventos de ingesta y tamaño del archivo **landing/origen**), por lo que **NO coinciden** con las pestañas Archivos/Peso, que miden los *part-files* y el tamaño real del parquet en **S3 staging** (el origen suele pesar ~2× el staging comprimido, y un load genera varios part-files). Son artefactos distintos del pipeline a propósito; están reetiquetados para no confundir. **Dependencias en el hub** (aplicadas una vez): bucket policy de lectura en `arc-ingestioncontrol`, permisos IAM Athena/Glue/S3 + `lakeformation:GetDataAccess` en el rol del hub, y un **grant Lake Formation SELECT/DESCRIBE** sobre la tabla de control (está gobernada por LF).
- **Frontend**: sub-módulo `frontend/src/scripts/modules/datalake.ts` (`createDatalakeModule(ctx)`), compuesto por el módulo Inicio que le delega render/eventos/gráfico de la sección. Toggle zona/métrica (**Archivos / Peso / Registros**), gráfico de barras diario, y dos vistas conmutables:
  - **Por área**: cada área → drill a **tabla diaria ordenable** (Día / Archivos / Peso); en métrica **Registros**, el drill muestra las **tablas** del área con archivos, peso y filas.
  - **Por fecha** (para investigar picos): cada día → drill a las **áreas ingestadas ese día**; en métrica **Registros** cada área se expande además a sus **tablas de ese día** (registros/ingestas/peso origen), cargadas **bajo demanda** vía `get_day_tables` (consulta Athena puntual por área+día, cacheada en items pequeños `…#recdaytbl#…`, porque el detalle área×tabla×día completo del rango no cabe en un item de 400KB). Se sirve por la **misma ruta** `/api/datalake/ingest/records` con params `area`+`day` (no se agregó ruta nueva: el resource policy del Lambda ya está al límite de 20KB). Además **clic en una barra del gráfico** salta a esa fecha y abre su desglose.
