...
|  Lambda Python | Toda la lógica de negocio. Un solo handler que enruta por path/método. También se auto-invoca asíncronamente para el sync global del catálogo. | `backend/app/handler.py` → `services/` → `repositories/` | Función `gestion-proyectos-dev-api` | Variables de entorno definidas en CDK (`MAIN_TABLE_NAME`, etc.) | Errores 5xx; revisar CloudWatch Logs  |
|  DynamoDB (single-table) | Tres cosas distintas en una tabla: (1) autorización funcional (usuarios, módulos), (2) datos operativos (personas, proyectos, tareas), (3) caché del catálogo Glue + contexto funcional de tablas/columnas. | `backend/app/repositories/` (`MainTableRepository`), modelo en `docs/04_modelo_dynamodb.md` | Tabla `gestion-proyectos-dev-main` | `MAIN_TABLE_NAME` | Sin permisos no hay módulos: usuario autenticado pero sin acceso a nada  |
...
- **Nivel 1 (servidor, persistente)**: el sync copia metadata de Glue a DynamoDB (`put_catalog_database/put_catalog_table/...`). El frontend siempre lee de esta caché. El sync global es asíncrono: `POST /api/catalog/sync` responde de inmediato y la Lambda se auto-invoca (`InvocationType=Event`, payload `{"action": "catalog_sync_all"}`); el badge "Sincronizando en background…" refleja eso. No hay EventBridge programado todavía.
