...
|  Lambda Python | Toda la lógica de negocio. Handler delgado que arma un **router por registro** (cada módulo registra sus rutas; autodescubrimiento con `pkgutil`). También se auto-invoca asíncronamente para el sync global del catálogo. | `backend/app/handler.py` → `core/router.py` → `modules/*_routes.py` → `services/` → `repositories/` | Función `gestion-proyectos-dev-api` | Variables de entorno en CDK (`MAIN_TABLE_NAME`, `COST_ACCOUNTS`) | Errores 5xx; revisar CloudWatch Logs  |
