...
|  DynamoDB (single-table) | Cinco cosas en una tabla: (1) autorización funcional (usuarios, módulos), (2) datos operativos (personas, proyectos, tareas), (3) caché del catálogo Glue + contexto funcional, (4) caché de costos AWS (`HOME#COSTS`), (5) caché del monitoreo de cargas del data lake (`DATALAKE#INGEST`). | `backend/app/repositories/` (**un repo por dominio**: `users`, `workspace`, `catalog`, `home`, `datalake`; base común en `base.py`), modelo en `docs/04_modelo_dynamodb.md` | Tabla `gestion-proyectos-dev-main` | `MAIN_TABLE_NAME` | Sin permisos no hay módulos: usuario autenticado pero sin acceso a nada  |
...
- **Sub-módulos**: una sección dentro de un módulo se extrae con el mismo patrón y la compone el módulo padre. Ejemplo: `datalake.ts` (monitoreo de cargas) lo compone `home.ts` (pestaña Data Lake), que le delega `sectionHtml()`/`bindEvents()`/`drawChart()`/`ensure()` y le pasa un callback `repaint`. Así Inicio no se vuelve un "archivo dios". Ver `docs/21_guia_nuevo_modulo.md`.
