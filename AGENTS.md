...
- **UI de módulos nuevos o rediseños**: seguir los 10 estándares visuales/usabilidad de `docs/06_frontend_ux.md` (objeto principal primero, maestro-detalle con tabla, una sola acción primaria, disciplina de color, acciones visibles con texto — drag&drop solo como atajo, empty states guiados, etc.). En maestro-detalle apilado (detalle debajo de la tabla), seleccionar una fila DEBE dar tres señales SIN robar el viewport: chevron ›/▾ por fila, "peek" (scroll mínimo para que el panel asome sin perder el listado; salto completo solo al hacer clic en el chevron) y destello breve del borde del panel (referencia: `revealProjectDetail(full)` en `workspace.ts`).
