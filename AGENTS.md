...
- **Datos**: DynamoDB single-table (autorización funcional, datos operativos y caché del catálogo) + Glue Catalog (metadata técnica, sincronizada a DynamoDB). El Catálogo es **multi-cuenta** (2026-07-15): selector con default en el hub `396913696127` (el Glue real de la plataforma; la cuenta app es réplica de pruebas) y llaves DynamoDB con namespace por cuenta (`docs/04` y `docs/07`). **Regla (2026-07-15): TODO selector de cuentas AWS arranca en el hub `396913696127`** — en las listas fuente del CDK (`costAccounts`, `catalogAccounts`) la PRIMERA entrada es la default y debe ser el hub.
